...
import { MapPin, Package, Truck, Clock, Search, CheckCircle } from "lucide-react";
    ...
    return (
      <div className="space-y-8 mt-6 animate-fade-in">
        <Card className="overflow-hidden border-transport-200 shadow-md hover:shadow-lg transition-shadow duration-300">
          <CardHeader className="bg-transport-100">
            <CardTitle className="text-xl text-transport-900">Order Details</CardTitle>
          </CardHeader>
        ...
        <Card className="overflow-hidden border-transport-200 shadow-md hover:shadow-lg transition-shadow duration-300">
          <CardHeader className="bg-transport-100">
            <CardTitle className="text-xl text-transport-900">Tracking Timeline</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {data.track.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <Package className="h-12 w-12 text-transport-300 mb-4" />
                <p className="text-gray-600">No tracking information available yet.</p>
                <p className="text-sm text-gray-500">Check back soon for updates on your shipment.</p>
              </div>
            ) : (
              <div className="relative">
                {data.track.map((item, index) => (
                  <div key={index} className="flex mb-8 relative group">
                    <div className="mr-4">
                      <div className="h-10 w-10 rounded-full bg-transport-100 flex items-center justify-center border-2 border-transport-700 group-hover:bg-transport-200 transition-colors duration-300">
                        {index === 0 ? (
                          <CheckCircle className="h-5 w-5 text-transport-700" />
                        ) : (
                    ...
                    <div className="flex-1 bg-white p-4 rounded-lg border border-transparent group-hover:border-transport-200 transition-all duration-300 shadow-sm hover:shadow-md">
                      <h4 className="font-medium text-transport-800 text-lg">{item.status}</h4>
        ...
        <div className="container mx-auto px-4 py-10">
          <div className="flex flex-col items-center">
            ...
            {!isLoading && !error && orderId && !data && (
              <div className="w-full max-w-lg bg-yellow-50 border border-yellow-200 text-yellow-800 p-6 rounded-md mt-6 animate-fade-in">
